...
//  Catches all our custom exceptions and returns structured JSON error responses.
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDto> handleBanned(UserBannedException ex, HttpServletRequest req) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ErrorResponseDto.of(403, "Forbidden", ex.getMessage(), req.getRequestURI()));
    }
